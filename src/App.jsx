    ...
    <div style={{position:"fixed",top:0,left:0,right:0,bottom:0,background:"rgba(0,0,0,0.85)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:20}}>
      <div style={{background:"#111",border:`1px solid ${C.border}`,borderRadius:12,padding:24,width:"100%",maxWidth:340}}>
        ...
        <div style={{fontSize:13,color:C.text,marginBottom:16}}>
          ¿Eliminar <span style={{color:C.gold}}>{item.name||item.desc||item.note||"este registro"}</span>?
        </div>
...
function Toast({ msg, onDone }) {
  ...
  return (
    <div style={{position:"fixed",bottom:24,left:"50%",transform:"translateX(-50%)",background:C.panel,border:`1px solid ${C.gold}`,borderRadius:8,padding:"10px 18px",fontSize:12,color:C.gold,zIndex:999,whiteSpace:"nowrap"}}>
      {msg}
    </div>
  );
}
    ...
    <div style={{display:"flex",gap:6,overflowX:"auto",marginBottom:16,paddingBottom:4}}>
      {MONTHS.map((m,i)=>(
        <button key={i} onClick={()=>onChange(i)} style={{
          flex:"0 0 auto",background:selected===i?C.gold:"none",
          border:`1px solid ${selected===i?C.gold:C.border}`,
          color:selected===i?C.bg:C.muted,
          borderRadius:20,padding:"4px 12px",fontSize:10,cursor:"pointer",fontFamily:"monospace"
        }}>{m}</button>
      ))}
    </div>
  );
...
export default function App() {
  ...
  const [confirmDel, setConfirmDel] = useState(null); // {item, key, list}
  ...
  function addAbono() {
    ...
    save({abonos:[{id:Date.now(),amount:parseFloat(aboAmt),note:aboNote.trim(),ts:Date.now(),by:currentUser},...abonos]}, `registró abono $${aboAmt}`);
    setAboAmt(""); setAboNote("");
  }

  function requestDelete(item, list, key, label) {
    setConfirmDel({item, list, key, label});
  }

  function confirmDelete(reason) {
    const {item, list, key} = confirmDel;
    const record = {
      id:Date.now(), original:item, key, reason,
      deletedBy:currentUser, deletedAt:Date.now()
    };
    save({[key]:list.filter(i=>i.id!==item.id), deleted:[record,...deleted]},
      `eliminó un registro (${reason})`);
    setConfirmDel(null);
  }

  function addPedido() {
    if (!pedName.trim()||!pedCost||isNaN(pedCost)) return;
    save({pedidos:[{id:Date.now(),name:pedName.trim(),costo:parseFloat(pedCost),abonos:[],ts:Date.now(),by:currentUser},...pedidos]}, `agregó pedido: ${pedName} $${pedCost}`);
    setPedName(""); setPedCost("");
  }
  function addAbonoPedido(pedId) {
    const amt=abonadoAmt[pedId]; if(!amt||isNaN(amt)) return;
    const updated=pedidos.map(p=>p.id===pedId?{...p,abonos:[...(p.abonos||[]),{id:Date.now(),amount:parseFloat(amt),note:abonadoNote[pedId]||"",ts:Date.now(),by:currentUser}]}:p);
    save({pedidos:updated}, `abonó $${amt} a pedido`);
    setAbonadoAmt({...abonadoAmt,[pedId]:""}); setAbonadoNote({...abonadoNote,[pedId]:""});
  }
  function delPedido(item) {
    setConfirmDel({item, list:pedidos, key:"pedidos", label:"pedido"});
  }
  ...
  function exportSummary() {
    ...
    const lines = [
      ...
      `🤝 ABONOS RECIBIDOS: $${fmt(abonos.reduce((a,b)=>a+b.amount,0))}`,
      `📦 SALDO PEDIDOS: $${fmt(pedidos.reduce((a,p)=>a+(p.costo-(p.abonos||[]).reduce((x,b)=>x+b.amount,0)),0))}`,
      ``,
    ...
    navigator.clipboard?.writeText(lines.join("\n")).then(()=>notify("✅ Resumen copiado al portapapeles")).catch(()=>notify("No se pudo copiar"));
  }
  ...
  const Row=({item,value,color,onDel})=>(
    <div style={{background:C.panel,border:`1px solid ${C.border}`,borderRadius:6,padding:"10px 12px",marginBottom:8,display:"flex",alignItems:"center",gap:8}}>
      <div style={{flex:1}}>
        <div style={{fontSize:10,color:C.muted,marginBottom:2,display:"flex",alignItems:"center"}}>{fDate(item.ts)}<ByTag by={item.by}/></div>
      ...
      <div style={{padding:"20px 20px 0"}}>
          ...
          <div>
            <div style={{textAlign:"center",marginBottom:24}}>
              ...
              <div style={{marginTop:24,display:"flex",justifyContent:"center",gap:12,flexWrap:"wrap"}}>
                {timerState==="idle"&&<button onClick={handleStart} style={{width:110,height:110,borderRadius:"50%",border:`2px solid ${C.gold}`,background:"rgba(200,169,110,0.07)",color:C.gold,fontSize:12,letterSpacing:3,textTransform:"uppercase",cursor:"pointer"}}>START</button>}
                {timerState==="running"&&<><button onClick={handlePause} style={{padding:"14px 24px",borderRadius:8,border:`2px solid ${C.blue}`,background:"rgba(126,168,200,0.07)",color:C.blue,fontSize:13,letterSpacing:2,textTransform:"uppercase",cursor:"pointer"}}>⏸ PAUSA</button><button onClick={handleFinalize} style={{padding:"14px 24px",borderRadius:8,border:`2px solid ${C.red}`,background:"rgba(200,126,126,0.07)",color:C.red,fontSize:13,letterSpacing:2,textTransform:"uppercase",cursor:"pointer"}}>■ FINALIZAR</button></>}
                {timerState==="paused"&&<><button onClick={handleContinue} style={{padding:"14px 24px",borderRadius:8,border:`2px solid ${C.green}`,background:"rgba(126,200,169,0.07)",color:C.green,fontSize:13,letterSpacing:2,textTransform:"uppercase",cursor:"pointer"}}>▶ CONTINUAR</button><button onClick={handleFinalize} style={{padding:"14px 24px",borderRadius:8,border:`2px solid ${C.red}`,background:"rgba(200,126,126,0.07)",color:C.red,fontSize:13,letterSpacing:2,textTransform:"uppercase",cursor:"pointer"}}>■ FINALIZAR</button></>}
              </div>
              {timerState==="running"&&<div style={{marginTop:12,display:"flex",alignItems:"center",justifyContent:"center",gap:6}}><div style={{width:6,height:6,borderRadius:"50%",background:pulse?C.gold:"transparent",border:`1px solid ${C.gold}`,transition:"background 0.5s"}}/><span style={{fontSize:9,color:C.muted,letterSpacing:2}}>EN CURSO</span></div>}
            </div>
        ...
        {tab===1&&(
          <div>
            ...
            <div style={{fontSize:9,color:C.dim,letterSpacing:3,marginBottom:12}}>REGISTROS · TOTAL ${fmt(filterByMonth(incomes,filterInc).reduce((a,i)=>a+i.amount,0))}</div>
        ...
        {tab===2&&(
          <div>
            ...
            <div style={{fontSize:9,color:C.dim,letterSpacing:3,marginBottom:12}}>REGISTROS · TOTAL ${fmt(filterByMonth(expenses,filterExp).reduce((a,e)=>a+e.amount,0))}</div>
        ...
        {tab===3&&(
          <div>
            ...
            <div style={{fontSize:9,color:C.dim,letterSpacing:3,marginBottom:12}}>ABONOS · TOTAL ${fmt(filterByMonth(abonos,filterAbo).reduce((a,b)=>a+b.amount,0))}</div>
              ...
              return (
                <div key={p.id} style={{background:C.panel,border:`1px solid ${saldo<=0?C.green:C.border}`,borderRadius:8,marginBottom:10,overflow:"hidden"}}>
                  <div style={{padding:"12px 14px",display:"flex",alignItems:"center",gap:8,cursor:"pointer"}} onClick={()=>setExpandedPed(expanded?null:p.id)}>
                    <div style={{flex:1}}>
                      <div style={{fontSize:10,color:C.muted,marginBottom:2,display:"flex",alignItems:"center"}}>{fDate(p.ts)}<ByTag by={p.by}/></div>
                    ...
                    {currentUser==="vendedor"&&<button onClick={e=>{e.stopPropagation();delPedido(p);}} style={{background:"none",border:`1px solid ${C.border}`,color:C.dim,borderRadius:4,padding:"3px 7px",cursor:"pointer",fontSize:11}}>✕</button>}
                    ...
                    <div style={{borderTop:`1px solid ${C.border}`,padding:"12px 14px"}}>
                      ...
                      {currentUser==="repartidor"&&saldo>0&&(
                        <div>
                          ...
                          <div style={{display:"flex",gap:8}}><input style={inp} placeholder="Monto USD" type="number" min="0" step="0.01" value={abonadoAmt[p.id]||""} onChange={e=>setAbonadoAmt({...abonadoAmt,[p.id]:e.target.value})}/><button onClick={()=>addAbonoPedido(p.id)} style={btn(C.gold)}>+ ADD</button></div>
                        </div>
                      )}
        ...
        {tab===5&&(
          <div>
            {[{label:"Ingresos en efectivo",value:totalIncome,color:C.green,sign:"+"},{label:`Horas trabajadas (${formatTime(totalSecs)})`,value:totalEarned,color:C.green,sign:"+"},{label:"Gastos",value:totalExp,color:C.red,sign:"−"},{label:"Abonos de sueldo",value:totalAbo,color:C.gold,sign:"−"}].map(item=>(
            ...
            <button onClick={exportSummary} style={{...btn(C.blue),width:"100%",marginTop:16,textAlign:"center"}}>📋 COPIAR RESUMEN</button>
            <div style={{background:C.panel,border:`1px solid ${C.border}`,borderRadius:8,padding:14,marginTop:12}}>
        ...
        {tab===6&&(
          <div>
            ...
            {deleted.length===0&&<div style={{color:C.dim,fontSize:12,textAlign:"center",padding:"16px 0"}}>Sin eliminaciones registradas</div>}
            {deleted.map(d=>(
              <div key={d.id} style={{background:C.panel,border:`1px solid ${C.border}`,borderRadius:6,padding:"10px 12px",marginBottom:8}}>
                <div style={{fontSize:10,color:C.muted,marginBottom:4,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                  <span>{fFull(d.deletedAt)}<ByTag by={d.deletedBy}/></span>
